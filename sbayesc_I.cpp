...
int main(int argc, char* argv[]) {
    ...
    if (argc < 6) {
        cerr << "Usage: " << argv[0] << "<GWAS summary file> <output beta file> <output nnz file> <random_seed> [n_iter] [pi_init] [hsq_init] [numSNP]" << endl;
        return 1;
    }

    // Read filenames and seed from command line
    string binFilePath = argv[1];
    string phenoFilePath = argv[1];
    string outputBetaFile = argv[2];
    string outputNnzFile = argv[3];

    // Random seed (from HPC job script)
    int random_seed = stoi(argv[4]);

    // Optional hyperparameters
    unsigned n_iter = (argc > 5) ? stoi(argv[5]) : 10000;
    float pi_init = (argc > 6) ? stof(argv[6]) : 0.1;
    float hsq_init = (argc > 7) ? stof(argv[7]) : 0.5;
    unsigned numSNP = (argc > 8) ? stof(argv[8]) : 1000;
    ...
    // Load data
    VectorXf b, se, n;
    MatrixXf LD = MatrixXf::Identity(numSNP, numSNP);
    ...
    for (int i = 1; i < n_iter; i++) {
        ...
        for (int j = 0; j < numSNP; j++) {
            ...
            float logDelta_inactive = log(1.0f - pi(i - 1));
