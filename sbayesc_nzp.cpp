...
void saveMatrixToBinary(const std::string& filename, const Eigen::MatrixXf& matrix) {
    ...
    // We need to save it in **row-major order** to match R's expectations.
    Eigen::MatrixXf rowMajorMatrix = matrix;
    file.write(reinterpret_cast<const char*>(rowMajorMatrix.data()), rows * cols * sizeof(float));
...
int main(int argc, char* argv[]) {
    ...
    unsigned n_iter = stoi(argv[7]) ;
    float pi_init = stof(argv[8]);
    float hsq_init = stof(argv[9]);
    ...
    VectorXf sigmaSq0 = VectorXf::Zero(n_iter+1);
    sigmaSq0(0) = (1.0/n_size) / (numSNP * (1.0-pi(0)));

    float nub = 4.0f, nue = 4.0f;
    float scaleb = (nub - 2) / nub * sigmaSq(0);
    float scaleb0 = (nub - 2) / nub * sigmaSq0(0);
    ...
    for (int i = 0; i < n_iter; i++) {
        ...
        float invSigmaSq0 = 1.0 / sigmaSq0(i);
        float logSigmaSq = log(sigmaSq(i));
        float logSigmaSq0 = log(sigmaSq0(i));
        ...
        for (int j = 0; j < numSNP; j++) {
            ...
            float invLhs0 = 1.0 / (1.0 / (vare / n_size) + invSigmaSq0);
            float uhat0 = invLhs0 * rhs;

            float logDelta_active = 0.5 * (log(invLhs1) - logSigmaSq + uhat1 * rhs) + logpi;
            float logDelta_inactive = 0.5 * (log(invLhs0) - logSigmaSq0 + uhat0 * rhs) + logpic;//-0.5*log(1.0/n_size) - 0.5*log(2*M_PI) - 0.5*beta_old*beta_old/n_size + logpic;
        ...
        float sigma_beta0 = sample_chisq(rng, numSNP - nnz(i) + nub);
        sigmaSq0(i+1) = (ssq0 + nub * scaleb0) / sigma_beta0;
    ...
    int start_row = 0.8*n_iter, end_row = n_iter;
    ...
    size_t pos = outputBetaFile.rfind(".bin");

    if (pos != std::string::npos) {
        outputBetaFile.erase(pos, 4);  
    }
